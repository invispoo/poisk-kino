<template>
    ...
    <div style="padding: 15px 0; text-align: center; margin: 0 auto">
            <a-row :gutter="[24, 24]">
                <a-col :span="4" v-for="(el, ind) in tabsList">
                    <router-link :to="'/film/' + index">
                        <film-card @id="index = $event" :element="addedFilm(ind)" :shortcutView="true"/>
                    </router-link>
                </a-col>
            </a-row>
...
<script>
    ...
    export default {
        ...
        methods: {
            addedFilm (index) {
                return this.json.find(film => film.id === +this.tabsList[index].id);
            }
        },
    }
