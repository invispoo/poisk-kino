        ...
        methods: {
            ...
            searchFilm() {
                if (this.searchInput === "") {
                    ...
                    if (this.isSorted) {
                        this.isSearched = false
                        this.searchedArray = this.selectSort(this.sortType).filter(
                            (film) =>
                                film.name
                                    .toLowerCase()
                        ...
                        this.searchedArray = this.json.filter((film) =>
                            film.name
                                .toLowerCase()
                                .includes(this.searchInput.toLowerCase())
                        )
                ...
                while (increment > 0) {
                    for (let i = increment; i < array.length; i++) {
                        ...
                        if (param2 !== undefined && sign === true) {
                            while (
                                j >= increment &&
                                array[j - increment][param1][param2] <
                                    temp[param1][param2]
                            ) {
                            ...
                            while (
                                j >= increment &&
                                array[j - increment][param1] < temp[param1]
                            ) {
                                array[j] = array[j - increment]
                                j = j - increment
                            }
